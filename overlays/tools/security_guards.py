...
ALLOWED_PERMISSIONS = {
    ...
    # job-hunt overlay: pipeline skills and tools (state machine, gates, guards).
    ...
    "Bash(uv run tools/verify_layout.py:*)",
    "Bash(uv run tools/security_guards.py:*)",
    "Bash(uv run tools/lint_skills.py:*)",
    "Bash(uv run pytest:*)",
    "Bash(pdflatex:*)",
    "Bash(xelatex:*)",
    "Bash(bun run src/cli.ts:*)",
...
REQUIRED_IGNORE_RULES = [
    ...
    "cv/main_*.*",
    # job-hunt overlay: upstream re-includes cv/main_example.tex; this private
    # workspace keeps the master CV (real PII) local-only instead.
    # job-hunt overlay: profile files and the template skeleton carry dense
    # PII (phone, full history) and stay local-only; a fresh clone runs /setup.
...
ALLOWED_IGNORE_NEGATIONS = {
    # job-hunt overlay: .env.* hides the (secret-free) template; re-include it.
    "!.env.example",
