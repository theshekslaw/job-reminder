...
> **Active template override: `abhishek-default`**
>
...
>
> - **Template skeleton:** `templates/cv/abhishek-default/template.tex` — use this as the structural reference instead of the stock template
> - **Manifest:** `templates/cv/abhishek-default/TEMPLATE.md` — read this for style rules and known pitfalls before drafting
> - **Source extension:** `.tex`
> - **Compile command:** `pdflatex -interaction=nonstopmode <file>` (not the lualatex command named in the stock guidance below — `/apply`'s compile step must use this instead)
> - **Fonts:** LaTeX defaults (Computer Modern); no fontspec, no bundled fonts
> - **Page limit:** exactly 1 page
> - **Output file:** `cv/main_<company>_<role>.tex`; the template is self-contained (no class/package/font files to copy)
<!-- END ACTIVE-TEMPLATE -->
